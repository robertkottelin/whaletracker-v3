...
const TetherContract = new Contract(TETHER_CONTRACT_ADDRESS, TETHER_ABI, provider);


const TRANSFER_THRESHOLD = ethers.utils.parseUnits('100', 'wei');
...
const App = () => {
  ...
  useEffect(() => {
    ...
    USDCContract.on('Transfer', handleTransfer);
    TetherContract.on('Transfer', handleTransfer);

    return () => {
      USDCContract.off('Transfer', handleTransfer);
      TetherContract.off('Transfer', handleTransfer);
    };
  }, []);
  ...
  return (
    <div className="App">
      <Table striped bordered hover>
        ...
        <tbody>
          {dataList.map(({ id, hash, from, to, amount }) => (
            <tr key={id}>
              <td>{id}</td>
              <td>{hash}</td>
              <td>{from}</td>
